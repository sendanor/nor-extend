...
extend.ActionObject = require('./ActionObject.js');

/* */
/*
function debug_call(name, args) {
	var util = require('util');
	var limit = 30;
	util.debug('CALL: '+name+' ( ' + args.map(function(s) {
		//if( util.inspect(s).length > limit) {
		//	return util.inspect(s, {colors:true}).substr(0, limit).replace("\n", " ").replace(/ +/, " ") + '...';
		//}
		return util.inspect(s, {colors:true}).replace(/[\n ]+/g, " ");
	//}).map(function(s) {
	//	return '{' + s + '}';
	}).join(', ') + ' )');
}
...
extend.object = function(self2, methods, obj) { // original extend_obj()
	...
	function get_all_methods(arr) {
		//debug_call('extend.object: get_all_methods(', Array.prototype.slice.call(arguments) );
		var ret = [];
...
extend.promise = function(methods, p) { // original extend_promise
	...
	/* Parse array of constructor functions */
	function get_all_methods(arr) {
		//debug_call('extend.promise: get_all_methods', Array.prototype.slice.call(arguments) );
		var ret = [];
		arr.forEach(function(m) {
			ret.push.apply(ret, extend.getMethodNamesFromConstructor(m) );
		});
	...
	function extend_if_promise(methods, ret) {
		...
		if(ret instanceof ExtendedPromise) {
			return ret;
		}
	...
	promise_methods.forEach(function(key) {
		...
		p2[key] = function() {
			...
			/*
			if(key === "then") {
				ret = p.then(function(obj) {
					var args2 = args.map(function(f) {
						if(!is.func(f)) { return f; }
						var f2 = function() {
							var f2_args = Array.prototype.slice.call(arguments);
							return f.apply(obj, f2_args);
						};
						return f2;
					});
					return p[key].apply(p, args2 );
				});
			} else {
			*/
